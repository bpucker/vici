...
def generate_heatmap(input_file, output_file, tab_separated=False):
    ...
    figure_height = max(0.25, num_genes * 0.1) + 0.001  # Increase height by 0.001 for summary brackets
    ...
    yticklabels=gene_names + '\n' + sequence_descriptions, center=0, cbar=True, vmin=-max_abs_z, vmax=max_abs_z, annot_kws={"fontsize": 35})  # Set vmin and vmax
    ...
    #y_labels = [f"{sequence}\n{gene}" for gene, sequence in zip(gene_names, sequence_descriptions)]
    #ax.set_yticklabels(y_labels, fontsize=20)  # Include gene names and sequence descriptions on the y-axis
    #ax.tick_params(axis='y', left=False)  # Hide y-axis ticks on the left
    ...
    for i, (gene, sequence) in enumerate(zip(gene_names, sequence_descriptions)):
        ...
        ax.text(-0.005, i + 0.6, gene, ha='right', va='center', fontsize=30, transform=ax.get_yaxis_transform())


        ax.text(-0.005, i + 0.4, sequence, ha='right', va='center', fontproperties=font_properties, transform=ax.get_yaxis_transform())
   ...
   # ax.set_ylim(0, len(gene_names))
    ...
    for i, (condition, replicate, culture) in enumerate(zip(conditions, replicates, cultures)):
        if condition != current_condition or culture != current_culture:
            ...
            ax.text((box_start + box_end) / 2, num_genes + 0.5, f"Condition: {current_condition}", ha='center', va='center', fontsize=30)
        ...
        ax.text(i + 0.5, num_genes + 0.2, f"{replicate}", ha='center', va='center', fontsize=35)
    ...
    for i, (condition, culture) in enumerate(zip(conditions, cultures)):
        if culture != current_culture:
            ...
            ax.text((box_start + box_end) / 1.0, num_genes + 1.0, f"Sample: {current_culture}", ha='center', va='center', fontsize=30)
    ...
    # Draw the last box
    #ax.add_patch(plt.Rectangle((box_start, num_genes - 0.08), box_end - box_start, num_genes + 2, fill=False, linewidth=2, edgecolor='blue'))
    ax.text((box_start + box_end) / 2, num_genes + 1.0, f"Sample: {current_culture}", ha='center', va='center', fontsize=30)
